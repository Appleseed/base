	...
	﻿namespace Appleseed.Services.Base.Engine.Processors.Impl
     {
         ...
         public class DropboxCloudFileCollector : ICollectThings
         {
             ...
             public DropboxCloudFileCollector(ILog logger, string connectionString, string tableName)
             {
             ...
             {
                 this.DropboxCollectionItems(items);
             }

             private const string DropboxSql = "SELECT * FROM {0} WHERE ItemKey IS NOT NULL AND ItemType IN ('{1}')"; //AND DATEDIFF(minute, ItemCreatedDate, GETDATE()) > 0
             private void DropboxCollectionItems(List<BaseItem> collectionPages)
             {
                 var sql = string.Format(DropboxSql, this.tableName, CollectionItemType.FILE);

                 this.Logger.Info("DropboxCollector: ConnectionString: " + this.connectionString);
                 this.Logger.Info("DropboxCollector: Table: " + this.tableName);
                 ...
                 using (var reader = SqlClientHelper.GetReader(this.connectionString, sql))
                 {
                     ...
                     // FilePath saved in ItemPath
                     var itemPathIndex = reader.GetOrdinal("ItemPath");
                     // ItemFilePath is empty
