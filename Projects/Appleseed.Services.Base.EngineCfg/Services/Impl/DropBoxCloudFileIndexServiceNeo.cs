...
namespace Appleseed.Services.Base.Engine.Services.Impl
{
    ...
    public class DropBoxCloudFileIndexServiceNeo : IAmABaseService
    {
        ...
        private readonly string userAgnet;


        public DropBoxCloudFileIndexServiceNeo(ILog logger, DropBoxCloudFileToIndexElement source)
        {
            if (logger == null)
            {
            ...
            {
                throw new ArgumentNullException("source.UserAgent");
            }
            ...
            //this.queueType = source.QueueName;
            ...
            {
                int start = source.QueueName.IndexOf('{');
                int end = source.QueueName.IndexOf('}');
                string name = source.QueueName.Substring(start + 1, end - start - 1);

                var queue = queueSection.Queue.FirstOrDefault(q => q.Name == name);

                if (queue != null)
                {
                    this.queueType = queue.Type;
                    this.queueConnectionString = queue.ConnectionString;
                    this.queueName = queue.QueueName;
                }
            }

            this.userAgnet = source.UserAgent;

        }

        public bool Run()
        {
            var indexer = new GeneralIndexer(this.logger, this.queueName);
            var aggregator = new DropBoxCloudFileAggregatorNeo(logger, this.sourceAuthAppKey, this.userAgnet, this.sourceAuthToken, this.sourceAuthTokenSecret, this.cacheFilesPath, this.queueConnectionString, this.queueType);
            var collector = new DropboxCloudFileCollector(logger, this.queueConnectionString, this.queueType);
