namespace Appleseed.Services.Base.Engine.Processors.Impl
{
    ...
    class CrunchBaseApiCrawlerCollector : ICollectThings
    {
        ...
        public CrunchBaseApiCrawlerCollector(ILog logger, string connectionString, string tableName)
        {
        ...
        {
            this.CrunchBaseCollectionItems(items);
        }

        private const string CrunchBaseSql = "SELECT * FROM {0} WHERE ItemKey IS NOT NULL AND ItemType IN ('{1}','{2}','{3}')"; //AND DATEDIFF(minute, ItemCreatedDate, GETDATE()) > 0
        private void CrunchBaseCollectionItems(List<BaseItem> collectionPages)
        {
            var sql = string.Format(CrunchBaseSql, this.tableName, CollectionItemType.CB_ORG, CollectionItemType.CB_PER, CollectionItemType.CB_PRO);

            this.Logger.Info("CrunchBaseCollector: ConnectionString: " + this.connectionString);
            this.Logger.Info("CrunchBaseCollector: Table: " + this.tableName);
                ...
                while (reader.Read())
                {
                    ...
                    switch (GetDbStringValue(reader, itemTypeIndex))
                    {
                        case CollectionItemType.NS_ACC:
                            ...
                            itemPath = GetDbStringValue(reader, itemFilePathIndex);
